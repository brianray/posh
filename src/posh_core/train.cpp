...
int train_citar_main(char* corpus, char* lexicon, char* ngrams)
{
	...
	ifstream corpusStream(corpus);
	if (!corpusStream.good())
	{
		cerr << "Could not open corpus for reading: " << corpus << endl;
		return 1;
	}

	brownCorpusReader.parse(corpusStream);

	ofstream lexiconStream(lexicon);
	if (!lexiconStream.good())
	{
		cerr << "Could not open lexicon for writing: " << lexicon << endl;
		return 1;
	}

	ofstream ngramStream(ngrams);
	if (!ngramStream.good())
	{
		cerr << "Could not open ngram list for writing: " << ngrams << endl;
		return 1;
	}
